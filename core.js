...
let _suppressLastMod = false;
function load() { try { return JSON.parse(localStorage.getItem(KEY)) || {}; } catch { return {}; } }
function save(d) {
  // Auto-stamp lastMod on any day/week that was opened via dayData()/weekData()
  // UNLESS suppressed (e.g. during loadAll re-render to prevent stale timestamps)
  if (!_suppressLastMod) {
  ...
  if (typeof FirebaseSync !== 'undefined' && !_suppressLastMod) FirebaseSync.onChange();
}
function suppressLastMod(val) { _suppressLastMod = val; }
...
function dayData(date) { _pendingDayMark = date; const d = load(); if (!d.days) d.days = {}; if (!d.days[date]) d.days[date] = { habits: {}, calBlocks: [], reflection: '', sealed: false }; return d; }
